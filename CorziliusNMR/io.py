...
def generate_csv_from_scream_set(path, output_file, expno, procno=103):
### Path to the Topspin files
    ...
    plt.savefig(output_file+".pdf", dpi='figure',format="pdf")
    np.savetxt((output_file+".csv"),npout,header=header,delimiter=",",comments = comment, fmt="%10.5f")
    plt.close()